...
def load_model():
    try:
        current_dir = Path(__file__).parent # Use __file__ to get the current file path
...
def validate_inputs(df):
    if 'Province name' in df.columns:
        ...
        df['Province name'] = df['Province name'].apply( lambda x: x if x in PROVINCES else 'Unknown' )
...
st.markdown("This tool assesses protest risk probability based on municipal characteristics.")

model = load_model()

if model is not None:
    # === Single Prediction Section ===
    st.header("Single Municipality Prediction")
...
# Footer
st.markdown("---")
st.caption("Protest Risk Prediction Model v1.0 Data sources: Census, Municipal Reports")                        
